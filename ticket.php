			   ...
			   <?php
					if($rparameters['ticket_type'])
					{
						...
						<div class="form-group row '; if((!$rright['ticket_type_disp'] && $_GET['action']!='new') || (!$rright['ticket_new_type_disp'] && $_GET['action']=='new')) {echo 'd-none';} echo'">
							...
							<div class="col-sm-5">
								<select style="min-width:269px;" id="type" name="type" class="form-control col-5" ';if(($rparameters['user_validation'] && $rparameters['user_validation_perimeter']=='mark' && $rright['ticket_user_validation']) || $rparameters['ticket_filter_type']) { echo ' onchange="loadVal(); submit();" ';} if($rright['ticket_type_mandatory']) { echo ' onchange="CheckMandatory();" ';} if(($rright['ticket_type']==0 && $_GET['action']!='new') || ($rright['ticket_new_type']==0 && $_GET['action']=='new') || $ticket_observer) {echo 'disabled="disabled"';} echo' >';
									//limit service type
				...
				<div class="form-group row <?php if(($rright['ticket_cat_disp']==0 && $_GET['action']!='new') || ($rright['ticket_new_cat_disp']==0 && $_GET['action']=='new')) echo 'd-none';?>">
					...
					<div class="col-sm-5">
						...
						<?php
							...
							//if type limit enable
							if($rparameters['ticket_filter_type'])
							{
								if($_POST['type'])
								{
									$where='WHERE (type="'.$_POST['type'].'" OR type="99999" OR id="0")';
								}elseif($globalrow['type'])
								{
									$where='WHERE (type="'.$globalrow['type'].'" OR type="99999" OR id="0")';
								}
							}
						...
						<!-- START plugins part --> 
						<?php
						$section='ticket_form_on_category';
						include('plugin.php');
						?>
						<!-- START plugins part --> 
					</div>
				...
				if($rparameters['ticket_places']==1)
				{
					...
					<div class="form-group row">
						<div class="col-sm-2 col-form-label text-sm-right pr-0">
							';
							//START plugins part
							$plugin_ticket_place_name=0;
							$section='ticket_place_name';
							include('plugin.php');
							if(!$plugin_ticket_place_name) {echo '<label class="mb-0" for="ticket_places">'.T_('Lieu').' :</label>';}
							//END plugins part 
							echo '
						</div>
						<div class="col-sm-5">
				...
				<!-- START plugins part --> 
				<?php
				$section='ticket_form_after_place';
				include('plugin.php');
				?>
				<!-- START plugins part --> 
				...
				<div class="form-group row <?php if($rright['ticket_state_disp']==0) echo 'd-none';?>">
					...
					<div class="col-sm-5">
						<select class="form-control <?php if($mobile){echo 'col-7';} else {echo 'col-5';} ?> d-inline-block" id="state"  name="state" <?php if(!$rright['ticket_state'] || $lock_tech==1 || $ticket_observer) echo 'disabled';?> >	
							...
							$qry=$db->prepare("SELECT `id`,`name` FROM `tstates` WHERE `id`!=:id1 AND `id`!=:id2 AND `hide`=0 ORDER BY `number`");
