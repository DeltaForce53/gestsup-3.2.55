...
if(!isset($sidebar_fixed)) {$sidebar_fixed=true;}
...
<div class="body-container" >
	...
	<nav class="navbar navbar-inverse navbar-expand-lg '; if($sidebar_fixed) {echo 'navbar-fixed';} echo ' '.$navbar.'">
	...
	<div class="main-container " style="'.$background_color.'">
		<div id="sidebar" class="overflow-auto sidebar '; if($sidebar_fixed) {echo 'sidebar-fixed';} echo ' expandable sidebar-color '.$sidebar.' '.$_SESSION['sidebar_collapsed'].'">
			';
			require('menu.php');
			echo '
		</div>
		...
		<div role="main" class="w-100 overflow-auto main-content">
			<div class="pt-2 page-content">
				...
				)
				{
					...
					elseif($_GET['page']=='ticket' && $_GET['action']!='new' && $rright['side_all_service_edit'] && $cnt_service!=0 && $user_services) {
					...
					elseif(($_GET['page']=='ticket' || $_GET['page']=='preview_mail') && $rright['side_all_agency_edit'] && $cnt_agency!=0 ) {
						//check if open ticket is associated to the same agency as the current user agencies
						$qry=$db->prepare("SELECT `u_agency` FROM `tincidents` WHERE id=:id");
						$qry->execute(array('id' => $_GET['id']));
						$check_ticket_agency=$qry->fetch();
					...
					elseif($_GET['page']=='dashboard' && $rright['side_group']) //group view access 
					{
						include("$_GET[page].php");
					}
