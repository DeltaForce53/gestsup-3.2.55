...
if(($_POST['user']=='%' || $_POST['user']=='%25') && $_POST['technician']!=$_SESSION['user_id']&& !$rright['side_all']&&!$_GET['companyview']&&!$_GET['techgroup']&&!$_GET['groupview']) {$_POST['user']=$_SESSION['user_id'];}
...
///// SQL QUERY
	...
	if($keywords && $reload==0)
	{
		...
		//special case to filter group query 
		if($rright['side_group'] && $_GET['groupview'])
		{
			$where.="
			AND 
			(
				tincidents.`t_group` IN (
					SELECT `tgroups_assoc`.`group` 
					FROM `tgroups_assoc`,`tgroups` 
					WHERE 
					`tgroups_assoc`.`group`=`tgroups`.`id` AND
					`tgroups_assoc`.`user`=$_SESSION[user_id] AND
					`tgroups`.`disable`='0'
				) OR 
				tincidents.`technician` IN (
					SELECT `tgroups_assoc`.`user` 
					FROM `tgroups_assoc` 
					WHERE `tgroups_assoc`.`group` IN (
						SELECT `tgroups_assoc`.`group` FROM `tgroups_assoc`,`tgroups`  
						WHERE 
						`tgroups_assoc`.`group`=`tgroups`.`id` AND
						`tgroups_assoc`.`user`=$_SESSION[user_id] AND
						`tgroups`.`disable`='0'
					)
					AND `tincidents`.`technician`!=$_SESSION[user_id]
				) OR `tincidents`.`technician`=$_SESSION[user_id]
			)
			";
		}
	}
...
<div class="page-header position-relative">
	<h1 class="page-title text-primary-m2">
		...
		else 
		{
			...
			elseif($_GET['groupview'] && $rright['side_group'])
			{
				//get group name for title
				//generate user 
				$qry=$db->prepare("SELECT `tgroups`.`name` FROM `tgroups`,`tgroups_assoc` WHERE `tgroups`.`id`=`tgroups_assoc`.`group` AND `tgroups_assoc`.`user`=:user AND `tgroups`.`disable`='0'");
				$qry->execute(array('user' => $_SESSION['user_id']));
				while($group=$qry->fetch()) 
				{
					if(!isset($user_groups)) {$user_groups=$group['name'];} else {$user_groups.=', '.$group['name'];}
				}
				$qry->closeCursor();
				$user_groups.=' <i title="'.T_("Liste des tickets associés à des groupes dans lesquels je suis présent, ou des tickets d'autres techniciens de mes groupes, ou des tickets sur lesquels je suis technicien").'" class="fa fa-question-circle text-70 text-primary-m2"></i>';
			    echo '<i class="fa fa-ticket text-primary-m2"><!----></i> '.T_('Tickets de mes groupes'). ' : '.$user_groups;
			}
			...
			<table id="simple-table" class="table table-bordered table-bordered table-striped table-hover text-dark-m2 " > 
				...
				<thead class="text-dark-m3 bgc-grey-l4">
					<tr class="bgc-white text-secondary-d3 text-95">
						...
						if(($_SESSION['profile_id']!=0 || $_SESSION['profile_id']!=4 || $_GET['userid']=='%') && $rright['dashboard_col_technician'])
						{
							echo '
							...
							if(($_SESSION['profile_id']!=0 || $_SESSION['profile_id']!=4 || $_GET['userid']=='%') && $rright['dashboard_col_technician'])
							{
								...
								<td>
									<select form="filter" style="width: 100% !important; min-width: 100%;" name="technician" id="technician" onchange="submit()" >
										...
										if($join)
										{
											$query="SELECT STRAIGHT_JOIN DISTINCT `tgroups`.`id`,`tgroups`.`name` FROM `tgroups` INNER JOIN `tincidents` ON `tgroups`.`id`=`tincidents`.`t_group` INNER JOIN tthreads ON tincidents.id=tthreads.ticket $join WHERE $where AND `tgroups`.`id`!=0 ORDER BY `tgroups`.`name`";
											//if(preg_match('#FROM tusers#',$query)) {$query=str_replace("LEFT JOIN tusers ON tincidents.user=tusers.id","",$query);} //avoid company column pb
											//if(preg_match('#INNER JOIN tcompany ON tusers.company=tcompany.id#',$query)) {$query=str_replace("INNER JOIN tcompany ON tusers.company=tcompany.id","",$query);} //avoid company column pb
											//if(preg_match("#AND tusers.company='$ruser[company]'#",$query)) {$query=str_replace("AND tusers.company='$ruser[company]'","",$query);} //fix empty technician filter on company view

										} else {
											$query = "SELECT `id`,`name` FROM `tgroups` WHERE `disable`='0' AND `type`='1' ORDER BY `name`";
										}
										$query = $db->query($query);
										while ($row = $query->fetch())
										{
				...
				<tbody>
					...
					if($reload==0)
					{
						...
						{ 
							...
							} elseif($rright['dashboard_col_date_create']) {
								$rowdate_create=date_create($row['date_create']);
								$rowdate_create_hour=date_format($rowdate_create, 'd/m/Y H:i');
