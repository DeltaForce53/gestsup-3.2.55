...
if($_POST['tenant']=='') $_POST['tenant']=$_GET['tenant'];
...
}elseif($rright['asset_list_tenant_only']) { //tenant
	$_POST['tenant']=$ruser['azure_ad_tenant_id'];
}
...
$url_post_parameters="sn_internal=$_POST[sn_internal]&ip=$_POST[ip]&netbios=$_POST[netbios]&user=$_POST[user]&type=$_POST[type]&model=$_POST[model]&description=$_POST[description]&department=$_POST[department]&location=$_POST[location]&tenant=$_POST[tenant]&virtual=$_POST[virtual]&date_stock=$_POST[date_stock]&date_end_warranty=$_POST[date_end_warranty]&state=$_POST[state]&warranty=$_GET[warranty]&assetkeywords=$assetkeywords&warranty_time=$_POST[warranty_time]&warranty_type=$_POST[warranty_type]";
...
if( 
	...
	($_POST['tenant']!='%' && $_POST['tenant']!='' && $_GET['tenant']!=$_POST['tenant'])||
...
if($assetkeywords)
{
	...
	$_POST['tenant']=str_replace("'","",strip_tags($db->quote($_POST['tenant'])));
...
if($rright['asset_list_col_tenant']!=0 && !$assetkeywords)
{
	$join.='LEFT JOIN tentra_tenant ON tassets.entra_id_tenant_id=tentra_tenant.tenant_id';
	if($_POST['tenant'] && $_POST['tenant']!='%'){$where.=" AND tentra_tenant.tenant_id LIKE '%$_POST[tenant]%' ";} else {$where.=" AND (tentra_tenant.tenant_id LIKE '%$_POST[tenant]%' OR tentra_tenant.tenant_id is null)";}
}
...
//add tenant limit
if($rright['asset_list_tenant_only'])
{
	$where.=" AND tusers.azure_ad_tenant_id='$ruser[azure_ad_tenant_id]'";
}
...
<div class="page-header position-relative">
	<h1 class="page-title text-primary-m2">
		...
		} elseif($rright['asset_list_company_only']) //for company view only
		{
			//get company name
		...
		} elseif($rright['asset_list_tenant_only']) //for tenant view only
		{
			//get company name
			$qry=$db->prepare("SELECT `tenant_name` FROM `tentra_tenant` WHERE tenant_id=:tenant_id");
			$qry->execute(array('tenant_id' => $ruser['azure_ad_tenant_id']));
			$row=$qry->fetch();
			$qry->closeCursor();
			if(!isset($row)) {echo 'error : no tenant defined on this user'; exit;}
			...
			<table id="simple-table" class="table table-bordered table-bordered table-striped table-hover text-dark-m2"> 
				...
				<thead>
					<tr class="bgc-white text-secondary-d3 text-95">
						...
						<?php
						if($rright['asset_list_col_location'] && $_POST['virtual']!=1)
						{
							echo '
						...
						if($rright['asset_list_col_tenant'])
						{
							echo '
							<th style="text-align:center;" ';if($_GET['order']=='tenant') echo 'class="active"'; echo ' >
								<a class="text-primary-m2"  title="'.T_('Locataire').'" href="'.$url.'&amp;order=tenant&amp;way='.$arrow_way.'">
									<i class="fa fa-building"></i><br />
									';
									echo T_('Locataire');
									//Display arrows
									if($_GET['order']=='tenant'){
										if($_GET['way']=='ASC') {echo ' <i class="fa fa-sort-up"></i>';}
										if($_GET['way']=='DESC') {echo ' <i class="fa fa-sort-down"></i>';}
									}
									echo '
					...
					<?php // *********************************** FILTER LINE ************************************** ?>
						<tr class="bgc-white text-secondary-d3 text-95">
							<td>
								<input form="filter" class="form-control form-control-sm" name="sn_internal" style="width:100%; margin:0 auto;" onchange="submit();" type="text" value="<?php if($_POST['sn_internal']!='%')echo $_POST['sn_internal']; ?>" />
							</td>
							<?php
								if($rparameters['asset_ip']==1)
								{
									echo '
									<td >
										<input form="filter" class="form-control form-control-sm" name="ip" style="width:100%; margin:0 auto;" onchange="submit();" type="text" value="'; if($_POST['ip']!='%') {echo $_POST['ip'];} echo '" />
									</td>
									';
								}
							?>
							<td>
								<input form="filter" class="form-control form-control-sm" name="netbios" style="width:100%; margin:0 auto" onchange="submit();" type="text" value="<?php if($_POST['netbios']!='%')echo $_POST['netbios']; ?>" />
							</td>
							<td >
								<select form="filter" class="form-control form-control-sm" name="user" style="width:80px; margin:0 auto;" onchange="submit()">
									<option value="%">&nbsp;</option>
									<?php
							...
							<td>
								<select form="filter" class="form-control form-control-sm" name="type" style="width:100px; margin:0 auto;" onchange="submit()">
									<option value="%">&nbsp;</option>
									<?php
							...
							<td>
								<select form="filter" class="form-control form-control-sm" style="width:90px; margin:0 auto;" name="model" onchange="submit()">
									<option value="%">&nbsp;</option>
									<?php
							...
							<td>
								<select form="filter" class="form-control form-control-sm" style="width:45px; margin:0 auto;" id="department" name="department" onchange="submit()">
									<option value="%">&nbsp;</option>
									<?php
							...
							if($rright['asset_list_col_location']!=0 && $_POST['virtual']!=1)
							{
								echo '
								<td>
									<input form="filter" class="form-control form-control-sm" name="location" onchange="submit();" style="width:70px; margin:0 auto;" type="text"  value="';if($_POST['location']!='%') {echo $_POST['location'];} echo '" />
								</td>
								';
							}
							if($rright['asset_list_col_tenant'])
							{
								echo '
								<td>
									<select form="filter" class="form-control form-control-sm" style="width:auto; margin:0 auto;" id="tenant" name="tenant" onchange="submit()">
										<option value="%">&nbsp;</option>
										';
										if($rright['asset_list_tenant_only'])
										{
											$query="SELECT tentra_tenant.tenant_name,tentra_tenant.tenant_id FROM tentra_tenant WHERE tentra_tenant.tenant_id=\"$ruser[azure_ad_tenant_id]\" ORDER BY tentra_tenant.tenant_name";
											$query = $db->query($query);
											while ($row=$query->fetch()){
												if($_POST['tenant']==$row['tenant_id']) {
													echo '<option selected value="'.$row['tenant_id'].'">'.$row['tenant_name'].'</option>';
												} else {
											...
											$query="SELECT tentra_tenant.tenant_name,tentra_tenant.tenant_id FROM tentra_tenant ORDER BY tentra_tenant.tenant_name";
											$query = $db->query($query);
											while ($row=$query->fetch()){
												if($_POST['tenant']==$row['tenant_id']) {
													echo '<option selected value="'.$row['tenant_id'].'">'.$row['tenant_name'].'</option>';
							...
							?>
							<td>
								<input form="filter" class="form-control form-control-sm" name="date_stock" onchange="submit();" style="width:82px; margin:0 auto;" type="text"  value="<?php if($_POST['date_stock']!='%') {echo $_POST['date_stock'];} ?>" />
							</td>
							<?php 
							if($rparameters['asset_warranty']==1 && $_GET['warranty']==1){echo '<td></td>';}
							?>
							<td>
								<select form="filter" class="form-control form-control-sm" style="width:50px; margin:0 auto;" id="state" name="state" onchange="submit()" >	
									<option value="%">&nbsp;</option>
									<?php
					...
					<?php
						...
						{ 
							...
							if($rright['asset_list_col_tenant'])
							{
								//get tenant name
								$qry=$db->prepare("SELECT `tenant_name` FROM `tentra_tenant` WHERE tenant_id=:tenant_id");
								$qry->execute(array('tenant_id' => $row['entra_id_tenant_id']));
								$rtenant=$qry->fetch();
								$qry->closeCursor();
								if(empty($rtenant['tenant_name'])) {$rtenant=array(); $rtenant['tenant_name']='';}
							}
