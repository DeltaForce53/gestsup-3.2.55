...
if($_GET['action']=="add")
{
	if($_GET['table']=='tcategory') //special case category
	{
		...
		$qry=$db->prepare("INSERT INTO `tcategory` (`name`,`service`,`technician`,`technician_group`,`type`) VALUES (:name,:service,:technician,:technician_group,:type)");
        $qry->execute(array('name' => $_POST['category'],'service' => $_POST['service'],'technician' => $_POST['technician'],'technician_group' => $_POST['technician_group'],'type' => $_POST['type']));
	}
