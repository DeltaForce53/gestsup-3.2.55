			...
			<table id="sample-table-1" class="table table-striped table-bordered table-hover">
						...
						{	
							if(($_GET['table']=='tcategory' || $_GET['table']=='tcriticality' || $_GET['table']=='tpriority' || $_GET['table']=='ttypes') && $rparameters['user_limit_service']==0 && $row['Field']=='service') {} else 
							{
								if($row['Field']!='ldap_guid')
								{
									...
									if($row['Field']=='hide') {$col_name=T_("Cacher");}
									...
									{
										//hide col if parameter is off
									}elseif($_GET['table']=='tcategory' && $row['Field']=='type' && !$rparameters['ticket_filter_type'])
									{
										//hide col if parameter is off
				...
				while ($row=$query->fetch()) 
				{
					...
					for($i=0; $i < $nbchamp1; ++$i)
					{
						...
						{
							//hide cols
						} elseif( //case 0 or 1 value replace by word
							...
							($_GET['table']=='tstates' && $i==7) ||
							($_GET['table']=='tstates' && $i==8) ||
						...
						elseif($_GET['table']=='tcategory' && $i==6 && !$rparameters['ticket_filter_type'])
						{
							//hide if ticket_filter_type disable
						}elseif($_GET['table']=='tcategory' && $i==6 && $rparameters['ticket_filter_type'])
						{
							...
							if($row[$i]=='99999') {$row2['name']=T_('Tous');}
