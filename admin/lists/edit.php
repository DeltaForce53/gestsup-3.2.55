...
if($_GET['action']=="update") 
{
	if($_GET['table']=='tcategory') //special case category
	{
		...
		//case ticket_filter_type 
		if($rparameters['ticket_filter_type'])
		{
			$qry=$db->prepare("UPDATE `tcategory` SET `type`=:type WHERE `id`=:id");
			$qry->execute(array('type' => $_POST['type'],'id' => $_GET['id']));
		}
                    ...
                    <div class="widget-body">
                        <div class="widget-main no-padding">
                            ...
                            ';
                                ...
                                {
                                    //find value
                                    $qry=$db->prepare("SELECT `name`,`number`,`technician`,`technician_group`,`type` FROM `tcategory` WHERE id=:id");
                                            ...
                                            echo '
                                                <div class="form-group">
                                                    ...
                                                    <select style="width:450px" class="form-control" name="type" id="type" >
                                                        <option value="99999">'.T_('Tous').'</option>
                                                        ';
                                                        $qry2=$db->prepare("SELECT `id`,`name` FROM `ttypes` ORDER BY `name`");
                                                        $qry2->execute();
                                                        while($type=$qry2->fetch()) 
                                                        {
                                                            echo '
                                                            <option value="'.$type['id'].'" 
                                                            '; if($row['type']==$type['id']) {echo 'selected';} echo '
                                                            >
                                                                '.$type['name'].'
                                                            </option>';
                                                        }
                                ...
                                elseif($_GET['table']=='tcompany') 
                                {
                                    ...
                                    echo '
                                        ...
                                        <div class="form-group">
                                            ...
                                            ';
                                            //list users of this company
                                            echo '
                                            <div class="form-group">
                                                <label for="users">'.T_('Utilisateurs de cette société').'</label>
                                                <ul>
                                                    ';
                                                    $qry=$db->prepare("SELECT `id`,`firstname`,`lastname` FROM `tusers` WHERE `company`=:company ORDER BY `lastname`" );
                                                    $qry->execute(array('company' => $_GET['id']));
                                                    while($user=$qry->fetch()) 
                                                    {
                                                        echo '<li><a href="index.php?page=admin&subpage=user&action=edit&userid='.$user['id'].'"> '.strtoupper($user['lastname']).' '.$user['firstname'].'</a></li>';
                                                    }
                                                    $qry->closeCursor();
                                                    echo '
                                                </ul>
                                ...
                                else //for all other table
                                {
                                    for ($i=1; $i <= $nbchamp; $i++)
                                    {
                                        ...
                                        if(${'champ' . $i}=='hide') {$label_name=T_("Cacher");}
