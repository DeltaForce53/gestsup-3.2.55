...
if($_POST['submit_general'] && $rright['admin'])
{
	...
	$qry=$db->prepare("
		...
		`ticket_filter_type`=:ticket_filter_type,
	...
	$qry->execute(array(
		...
		'ticket_filter_type' => $_POST['ticket_filter_type'],
		'ticket_default_state' => $_POST['ticket_default_state'],
		'meta_state' => $_POST['meta_state'],
		'id' => '1'
		));

    //update advanced user parameters if company_message enable
    if($_POST['company_message']==1)
    {
        $qry=$db->prepare("UPDATE `tparameters` SET `user_advanced`='1'");
        $qry->execute();
    }
                        ...
                        <div class="tab-content p-35 border-1 brc-grey-l1 shadow-sm bgc-white">
                            ...
                            <div class="tab-pane fade text-95 <?php if($_GET['subtab']=='display') {echo 'show active';}?> ">
                                <div class="text-primary-d3 text-110 mb-2">
                                    ...
                                    <label class="lbl" ><?php echo T_("Fond d'écran sur la page de connexion"); ?> : </label>
                                    <?php
                                    ...
                                    <div class="pt-1"></div>
                                    <label class="lbl" for="tab_name"><?php echo T_("Nom de l'onglet"); ?> :</label>
                                    <input style="width:auto" class="form-control d-inline-block" name="tab_name" id="tab_name" type="text" value="<?php echo $rparameters['tab_name']; ?>" size="15" />
                                    <i data-toggle="tooltip" id="tooltip77" data-placement="auto" data-original-title="<?php echo T_("Permets de modifier le titre de l'onglet du navigateur, exemple : GestSup | mon texte"); ?>" class="fa fa-question-circle text-primary-m2"><!----></i>
                            ...
                            <div class="tab-pane fade text-95 <?php if($_GET['subtab']=='ticket') {echo 'show active';}?> ">
                                <div class="text-primary-d3 text-110 mb-2">
                                    ...
                                    <div class="pt-1"></div>
                                    <label  class="lbl">
                                        <input type="checkbox" <?php if($rparameters['ticket_filter_type']) {echo "checked";} ?> name="ticket_filter_type" id="ticket_filter_type" value="1">
                                        <span class="lbl"><?php echo T_("Filtre de la catégorie en fonction d'un type"); ?></span>
                                        <i data-toggle="tooltip" id="tooltip76" data-placement="auto" data-original-title="<?php echo T_("Permet de filtrer la liste des catégories en fonction d'un type de ticket, à définir dans Administration > Lites > Catégorie"); ?>" class="fa fa-question-circle text-primary-m2"><!----></i>
                                    </label>
                                </div>
                            </div>
                            <!-- subtab sound -->
                            ...
                            <div class="tab-pane fade text-95 <?php if($_GET['subtab']=='user') {echo 'show active';}?> ">
                                <div class="text-primary-d3 text-110 mb-2">
                                    ...
                                    <br />
                                    <label class="lbl"> 
                                        <input id="user_mfa" name="user_mfa" type="checkbox" <?php if($rparameters['user_mfa']) {echo "checked";} ?> value="1">
                                        <span class="lbl">&nbsp;<?php echo T_("Connexion multifacteur (mail)"); ?></span>
                                        <i data-toggle="tooltip" id="tooltip75" data-placement="auto" data-original-title="<?php echo T_("Lors de la connexion un mail est émit à l'adresse mail, afin d'obtenir un code d'accès"); ?>" class="fa fa-question-circle text-primary-m2"><!----></i>
                                    </label>
                                </div>
                            </div>
                            <!-- subtab mail -->
