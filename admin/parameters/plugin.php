...
if($_POST['submit_plugin'] && $rright['admin'])
{
    ...
    foreach ($plugins as $plugin_scan)
    {
        ...
        $qry=$db->prepare("SELECT `enable` FROM `tplugins` WHERE `name`=:name");
        $qry->execute(array('name' => $plugin_scan));
        $plugin_state=$qry->fetch();
        $qry->closeCursor();

       if(isset($plugin_state['enable'])) //if plugin folder
       {
            if($plugin_state['enable']!=isset($_POST[$plugin_scan])) //if plugin modification is detected
            {
                ...
                //redirect
                $www = './index.php?page=admin&subpage=parameters&tab=plugin&subtab='.$_GET['subtab'].'&id='.$_GET['id'];
...
if(($_POST['install_store_plugin'] || $_POST['install_licenced_plugin']) && $rright['admin'])
{
    ...
    if(isset($plugin_filename) && isset($plugin_code))
    {
        ...
        if($plugin_licenced)
        {
            $url = "https://zensoft.fr/plugins/$rparameters[server_private_key]/$plugin_filename";
        } else {
        ...
        if(file_exists($file_local_url)) 
        {
            ...
            if(file_exists(__DIR__ ."/../../plugins/$plugin_code/changelog.php"))
            {
                ...
                echo DisplayMessage('error',T_("L'extraction du plugin à échouée"));
                //remove download file
                unlink($file_local_url);
            }
...
<div id="plugin" class="tab-pane <?php if($_GET['tab']=='plugin') echo 'active'; ?>">
    ...
    <form id="plugin_form" name="plugin_form" enctype="multipart/form-data" method="POST" action="" onsubmit="loadVal();">
                                        ...
                                        foreach($plugin_remote_array as $plugin)
                                        {
                                            ...
                                            echo '
                                                <div class="shadow-sm mt-3 bgc-purple-l4 pos-rel overflow-hidden radius-2 pt-25 pb-1 px-4 border-1 brc-purple-l2">
                                                                ...
                                                                if($plugin['free']==0)
                                                                {
                                                                    ...
                                                                    if($plugin['licenced'])
                                                                    {
                                                                        ...
                                                                        echo '
                                                                            ...
                                                                            <button title="'.T_('Ouvre la version de démonstration').'" onclick="window.open(\'https://demo.gestsup.fr\',\'_blank\')" class="ml-2 btn btn-xs btn-info">
                                                    ...
                                                    <div class="pos-rel">
                                                        ';
                                                        $plugin['description']=str_replace('<br />','',$plugin['description']);
                                                        echo '
                                                        '.$plugin['description'].'
                                                    </div>
                                                    <div class="mt-2 pos-rel">
