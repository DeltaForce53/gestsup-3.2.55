...
if($rparameters['azure_ad'] && $_GET['delete'] && $_GET['subtab']=='azure' && $rright['admin'])
{
    ...
    //redirect
	$www = "./index.php?page=admin&subpage=parameters&tab=connector&subtab=$_GET[subtab]";
...
if($rright['admin'] && ($_POST['submit_connector'] || $_POST['test_ldap'] || $_POST['test_smtp'] || $_POST['test_imap'] || $_POST['test_azure_ad']))
{
    ...
    {
        //redirect
        $www = './index.php?page=admin&subpage=parameters&tab=connector&subtab='.$_GET['subtab'].'&ldaptest='.$_POST['test_ldap'].'&smtptest='.$_POST['test_smtp'].'&test_azure_ad='.$_POST['test_azure_ad'];
        echo '<script language="Javascript">
        <!--
        document.location.replace("'.$www.'");
        // -->
                                    ...
                                    <div id="imap_parameters">
                                        ...
                                        echo '
                                            ...
                                            <select style="width:auto" class="form-control  d-inline-block" id="imap_inbox" name="imap_inbox" >
                                                ...
                                                <option ';if($rparameters['imap_inbox']=='gestsup') {echo "selected ";} echo ' value="gestsup">gestsup</option>
                                ...
                                <div class="text-primary-d3 text-110 mb-2">
                                    ...
                                    <div id="azure_ad_parameters">
                                        ...
                                        for($i = 1; $i <= $rparameters['azure_ad_tenant_number']; $i++) {
                                            ...
                                            if(!isset($tenant['id'])) {$tenant=array(); $tenant['id']='';}
                                            if(!isset($tenant['tenant_name'])) {$tenant['tenant_name']='';}
                                            if(!isset($tenant['client_id'])) {$tenant['client_id']='';}
                                            if(!isset($tenant['tenant_id'])) {$tenant['tenant_id']='';}
                                            if(!isset($tenant['group_filter'])) {$tenant['group_filter']='';}
