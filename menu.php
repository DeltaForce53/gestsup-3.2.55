...
<div class="sidebar-inner">
	<div class="flex-grow-1 ace-scroll ">
		...
		';
				...
				<li "; if($selected_side_your) {echo 'class="nav-item active open"';} else {echo 'class="nav-item"';}  echo ">
					...
					<div class="hideable submenu collapse '; if($selected_side_your) {echo 'show';} echo '">
						<ul class="submenu-inner" >';
							...
							$qry=$db->prepare("SELECT `id`,`description`,`name`,`icon` FROM `tstates` WHERE `id`!=5 AND `hide`=0 ORDER BY `number`");
							$qry->execute();
							while($row=$qry->fetch()) 
							{
			...
			//display side menu for group view
			if($rright['side_group'])
			{
				//check current user is member of one group
				$qry=$db->prepare("SELECT `id` FROM `tgroups_assoc` WHERE user=:user");
				$qry->execute(array('user' => $_SESSION['user_id']));
				$row=$qry->fetch();
				$qry->closeCursor();
				if(!empty($row))
				{
					if(($_GET['page']=='dashboard' || $_GET['page']=='ticket') && $_GET['groupview']) {$groupview=1;} else {$groupview=0;}

					//get counters
					$qry=$db->prepare("SELECT COUNT(`id`) AS counter 
						FROM `tincidents` 
						WHERE `disable`='0' 
						AND (
							`t_group` IN (
								SELECT `tgroups_assoc`.`group` 
								FROM `tgroups_assoc`,`tgroups`
								WHERE 
								`tgroups_assoc`.`group`=`tgroups`.`id` AND
								`tgroups_assoc`.`user`=:user AND
								`tgroups`.`disable`='0'
							) OR 
							`technician` IN (
								SELECT `user` FROM `tgroups_assoc` WHERE `group` IN (
									SELECT `group` 
									FROM `tgroups_assoc`,`tgroups` 
									WHERE 
									`tgroups_assoc`.`group`=`tgroups`.`id` AND
							...
							`technician`!=:user ) OR 
							`tincidents`.`technician`=:user
							);"
						);
					$qry->execute(array('user' => $_SESSION['user_id']));
					$cnt_grp_all=$qry->fetch();
					$qry->closeCursor();

				
					if($groupview){echo '<li class="nav-item active open" >';} else {echo '<li class="nav-item" >';}  
					echo '
						<a href="./index.php?page=dashboard&amp;userid=%25&amp;state=%25" class="nav-link dropdown-toggle">
							<i class="nav-icon fa fa-ticket"><!----></i>
							<span class="nav-text fadeable"> 
								'.T_('Tickets de groupe').'
							</span>
							<b class="caret fa fa-angle-left rt-n90"></b>
						</a>
						<div class="hideable submenu collapse '; if($groupview) {echo 'show';} echo ' ">
							<ul class="submenu-inner" >';
								if($_GET['page']=='dashboard' && $_GET['userid']=='%' && $_GET['state']=='%') {echo '<li class="nav-item active">';} else {echo '<li class="nav-item">';}
								echo '
									<a class="nav-link pl-4" href="./index.php?page=dashboard&amp;userid=%25&amp;state=%25&amp;ticket=%25&amp;technician=%25&amp;user=%25&amp;category=%25&amp;subcat=%25&amp;title=%25&amp;date_create=%25&amp;priority=%25&amp;criticality=%25&amp;groupview=1">
										<i class="nav-icon fa fa-angle-right pr-1"><!----></i>
										'.T_('Tous les états').' ('.$cnt_grp_all[0].')
									</a>
								</li>';
								//display meta states link
								if($rparameters['meta_state']==1  && $rright['side_all_meta']!=0)
								{
									$qry=$db->prepare("SELECT COUNT(`id`) AS counter 
										...
										`disable`=0 AND 
										`state` IN (
											SELECT `id` FROM `tstates` WHERE `meta`='1'
										) AND (
											`t_group` IN (
												SELECT `tgroups_assoc`.`group`
												FROM `tgroups_assoc`,`tgroups`  
											...
											)  OR `tincidents`.`technician`=:user
										);
									");
									$qry->execute(array('user' => $_SESSION['user_id']));
									$cnt_grp_meta=$qry->fetch();
									$qry->closeCursor();  
									
									if($_GET['page']=='dashboard' && $_GET['userid']=='%' && $_GET['state']=='meta') {echo '<li class="nav-item active">';} else {echo '<li lass="nav-item">';}
									echo '
										<a class="nav-link pl-4" title="'.$label_meta.'" href="index.php?page=dashboard&amp;userid=%25&amp;state=meta&amp;ticket=%25&amp;technician=%25&amp;user=%25&amp;category=%25&amp;subcat=%25&amp;title=%25&amp;date_create=%25&amp;priority=%25&amp;criticality=%25&amp;groupview=1">
											<i class="nav-icon fa fa-angle-right pr-1"><!----></i>
											'; echo T_('À traiter'); echo' ('.$cnt_grp_meta['counter'].')
										</a>
									</li>';
								}	
								//for each state display in sub-menu
								$qry=$db->prepare("SELECT `id`,`name`,`description` FROM `tstates` WHERE `id`!=5 AND `hide`=0 ORDER BY `number`");
								$qry->execute();
								while($row=$qry->fetch()) 
								{
									$qry2=$db->prepare("SELECT COUNT(`id`) AS counter 
										FROM `tincidents` 
										WHERE 
										`disable`=0 AND 
										`state`=:state AND (
									...
									$qry2->execute(array('state' => $row['id'],'user' => $_SESSION['user_id']));
									$cnt=$qry2->fetch();
									$qry2->closeCursor();
									
									if($_GET['page']=='dashboard' && $_GET['userid']=='%' && $_GET['state']==$row['id']) {echo '<li class="nav-item active">';} else {echo '<li class="nav-item">';} 
									echo '
										<a class="nav-link pl-4" title="'.T_($row['description']).'" href="index.php?page=dashboard&amp;userid=%25&amp;state='.$row['id'].'&amp;ticket=%25&amp;technician=%25&amp;user=%25&amp;category=%25&amp;subcat=%25&amp;title=%25&amp;date_create=%25&amp;priority=%25&amp;criticality=%25&amp;groupview=1">
											<i class="nav-icon fa fa-angle-right pr-1"><!----></i>
											'.T_($row['name']).' ('.$cnt[0].')
										</a>
									</li>';
			...
			{
				if(($_GET['userid']=='%' || $_GET['userid']=='0') && $_GET['viewid']=='' && $_GET['companyview']==''&& $_GET['groupview']=='') {$side_all_ticket_selected=1;} else {$side_all_ticket_selected=0;}
									...
									if($side_view_selected) {echo '<li class="nav-item active open" >';} else {echo '<li class="nav-item" >';}  
										...
										<div class="hideable submenu collapse '; if($side_view_selected) {echo 'show';} echo '">
											<ul class="submenu-inner">
												...
												//for each state display in sub-menu
												$qry2=$db->prepare("SELECT `id`,`name`,`description` FROM `tstates` WHERE `id`!=5 AND `hide`=0 ORDER BY `number`");
												$qry2->execute();
			...
			if($rright['asset'] && $rparameters['asset'])
			{
					...
					if($rright['side_asset_all_state']!=0)
					{
						...
						<div class="hideable submenu collapse '; if($side_asset_selected) {echo 'show';} echo '">
							<ul class="submenu-inner">
								...
								} elseif($rright['asset_list_tenant_only'])
								{
									$qry=$db->prepare("SELECT COUNT(tassets.id) FROM `tassets`,`tusers` WHERE `tassets`.`user`=`tusers`.`id` AND `tassets`.`disable`='0' AND `tusers`.`azure_ad_tenant_id`=:tenant_id");
								...
								while($row=$qry->fetch()) 
								{
									...
									{
										$qry2=$db->prepare("SELECT COUNT(tassets.id) FROM `tassets`,`tusers` WHERE tassets.user=tusers.id AND tassets.state=:state AND tassets.disable='0' AND tusers.azure_ad_tenant_id=:tenant_id");
										$qry2->execute(array('state' => $row['id'],'tenant_id' => $ruser['azure_ad_tenant_id']));
										$cnt=$qry2->fetch();
										$qry2->closeCursor();
				...
				echo '
					...
					<div class="hideable submenu collapse '; if($_GET['page']=='admin' || $_GET['page']=='changelog') {echo 'show';} echo ' ">
						<ul class="submenu-inner">';
							...
							if($rright['admin'])
							{
								if($rright['admin_backup'] && $rparameters['server_private_key']!='213915b2a92a7ce7650dc7c3c120371')
								{
