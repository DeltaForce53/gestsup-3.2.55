...
if($_GET['page']!='register' && $_GET['page']!='forgot_pwd' && $_GET['page']!='login' && $_GET['page']!='mfa') {session_name(md5($_SERVER['SERVER_NAME'].$_SERVER['PHP_SELF']));}
...
//auth token using in azure ad and saml2 plugin and MFA
...
//check php version
if(phpversion()<8.1) {echo DisplayMessage('error', T_('Version de PHP non supportée, veuillez mettre à jour votre version de PHP 8.1 minimum'));};
...
if($_POST['userkeywords']||$_GET['userkeywords']) {
	...
	$userkeywords=clean($userkeywords);
} else {$userkeywords='';}
if($_POST['assetkeywords']||$_GET['assetkeywords']) {
	...
	$assetkeywords=clean($assetkeywords);
} else {$assetkeywords='';}
if($_POST['rightkeywords']||$_GET['rightkeywords']) {
	...
	$rightkeywords=clean($rightkeywords);
} else {$rightkeywords='';}
if($_POST['procedurekeywords']||$_GET['procedurekeywords']) {
	...
	$procedurekeywords=clean($procedurekeywords);
...
if($_POST['listkeywords']||$_GET['listkeywords']) {
	...
	$listkeywords=clean($listkeywords);
} else {$listkeywords='';}
...
<html lang="fr" style="--scrollbar-width:17px; --moz-scrollbar-thin:17px; font-size: 0.925rem;">
	<head>
		...
		<title>
			GestSup | 
			<?php if($rparameters['tab_name']) {echo $rparameters['tab_name'];} else {echo T_('Gestion de Support');} ?>
		</title>
		...
		if($_GET['page']=='ticket' || $_GET['page']=='asset' || $_GET['page']=='test' || $_GET['table']=='tcompany'|| $_GET['tab']=='plugin' || ($_GET['page']=='dashboard' && $_GET['view']=='activity')) 
		{
			echo '
	...
	<?php
		...
		if($_SESSION['user_id']) 
		{
			...
			if($rparameters['cron_weekly']!=date("W", time())) {require('./core/cron_weekly.php');}
			...
			if($rparameters['ldap_sso'] && isset($_SERVER['REMOTE_USER']) && $_GET['action']!='logout' && $ipcheck==1)
			{
				...
				elseif($_GET['page']=='mfa') {require('mfa.php');}
