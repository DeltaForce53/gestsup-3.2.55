...
$qry=$db->prepare("SELECT `name` FROM `ttime` WHERE min=:min");
$qry->execute(array('min' => $globalrow['time']));
$timerow=$qry->fetch();
$qry->closeCursor();
if(!isset($timerow['name'])) {$timerow=array(); $timerow['name']='';}
...
if(!isset($object))
{
	if($_POST['close']){$globalrow['state']=3;}
...
if(file_exists($template_filename))
{
	...
	$mail_template=str_replace('#ticket_time#', $timerow['name'], $mail_template);
