    ...
    if($_GET['action']=='simul' || $_GET['action']=='run' || php_sapi_name() == "cli"){
        ...
        for($id = 1; $id <= $rparameters['azure_ad_tenant_number']; $id++) 
        {
            ...
            if(!$accessToken)
            {
                ...
                //fields list
                $qry_select = 'id,onPremisesSecurityIdentifier,onPremisesSamAccountName,onPremisesUserPrincipalName,givenName,surname,displayName,jobTitle,department,businessPhones,mobilePhone,mail,companyName,userPrincipalName,streetAddress,postalCode,city,faxNumber,accountEnabled';
