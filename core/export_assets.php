...
//secure var
$db_userid=strip_tags($_GET['user_id']);
...
//check var
if(!is_numeric($db_userid) && $db_userid!='%') {echo 'ERROR : incorrect value user_id'; exit;}
if(!is_numeric($db_technician) && $db_technician!='%') {echo 'ERROR : incorrect value technician'; exit;}
if(!is_numeric($db_service) && $db_service!='%') {echo 'ERROR : incorrect value service'; exit;}
if(!is_numeric($db_month) && $db_month!='%') {echo 'ERROR : incorrect value month '; exit;}
if(!is_numeric($db_company) && $db_company!='%') {echo 'ERROR : incorrect value company'; exit;}
if(!is_numeric($db_model) && $db_model!='%') {echo 'ERROR : incorrect value model'; exit;}
if(!is_numeric($db_netbios) && $db_netbios!='%') {echo 'ERROR : incorrect value netbios'; exit;}
...
$token=$qry->fetch();
$qry->closeCursor();
if(empty($token['id'])) {echo "ERROR : Wrong token $_SERVER[REMOTE_ADDR]"; exit;}
