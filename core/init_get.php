...
//check page 
$page_white_list=array('dashboard','ticket','preview_mail','asset_list','asset','asset_stock','procedure','calendar','project','stat','admin','admin/user','changelog','register','forgot_pwd','plugin','contract','test','core/ocs','mfa','core/mfa','login');
...
//plugin
$section='init_get_order';
if(file_exists('plugin.php')) {include('plugin.php');}
...
foreach($all_numeric_var as $numeric_var) {
	if(
		$_GET[$numeric_var] &&
		!is_numeric($_GET[$numeric_var]) &&
		$_GET[$numeric_var]!='%' &&
		$_GET[$numeric_var]!='all' &&
		$_GET[$numeric_var]!='meta'
	) {echo 'ERROR : incorrect value on $_GET['.$numeric_var.']'; exit;}
}
