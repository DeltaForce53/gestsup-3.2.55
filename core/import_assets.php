			...
			if($extension=='csv')
			{
				...
				while (($line = fgetcsv($file)) !== FALSE) 
				{
					...
					if($read_line[0]!="Numéro de l'équipement") //don't show first line if title line is detected
					{
						...
						$user_firstname=strip_tags($read_line[10]);
						$user_lastname=strip_tags($read_line[11]);
						$state=strip_tags($read_line[12]);
						$department=strip_tags($read_line[13]);
						$location=strip_tags($read_line[14]);
						$date_install=$read_line[15];
						$date_end_warranty=$read_line[16];
						$date_stock=$read_line[17];
						$date_standbye=$read_line[18];
						$date_recycle=$read_line[19];
						$date_last_ping=$read_line[20];
						$socket=strip_tags($read_line[21]);
						$technician=strip_tags($read_line[22]);
						$maintenance=strip_tags($read_line[23]);
						...
						if($mac)
						{
							...
							if(isset($row['mac']))
							{
								...
								if($user_firstname && $user_lastname) {
									...
									$csv_firstname=$user_firstname;
									$csv_lastname=$user_lastname;
									...
									if(($gs_firstname!=$csv_firstname) || ($gs_lastname!=$csv_lastname) )
									{
										...
										$row3=$qry3->fetch();
										$qry3->closeCursor();
										$msg="[$gs_mac_disp] -update user for asset_id $gs[id] with user_id=$row3[0] $user_firstname $user_lastname";
										if($argv[0]){echo $msg.PHP_EOL;logit('asset_import',$msg,'0');}
										else {if($rparameters['debug']) {echo "$msg<br />";}}
